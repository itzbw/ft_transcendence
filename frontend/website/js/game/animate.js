...
export function animate_vs_bot(gameData) {
	...
	function animateLoop() {
		if (gameData.isAnimating) {
			...
			gameData.animFrameId = requestAnimationFrame(animateLoop);
			const delta = gameData.clock.getDelta();
			const elapsedTime = gameData.clock.getElapsedTime();
			...
			function onGoal() {
				...
				if (leftWon || rightWon) {
					...
					destroy(gameData.scene, gameData.animFrameId, gameData.gui);
				}
			}
		...
		window.addEventListener('hashchange', function () {
			destroy(gameData.scene, gameData.animFrameId);
		});
	}
